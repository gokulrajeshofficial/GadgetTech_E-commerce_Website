...
module.exports = {
    ...
    brandProductsCount :(brandId)=>{
        return new Promise(async (resolve, reject) => {
            let count = await db.get().collection('product').find({ Brand : objectId(brandId)}).toArray()
            console.log(count.length)
            resolve(count.length)
        })
    },
    brandProducts: (brandId, page) => {
        return new Promise(async (resolve, reject) => {
            let products = await db.get().collection('product').aggregate([
                {
    ...
    updateProduct: (product, productId) => {
        ...
        return new Promise(async (resolve, reject) => {
            console.log(product)
            await db.get().collection('product').updateOne({ _id: objectId(productId) }, {
                $set: {
                    Name: product.Name,
